...
' BEGIN IF NEW.accepted = False THEN NEW.accepted = False; ELSIF NEW.accepted = True AND NEW.username_caretaker IN (SELECT username FROM caretakers_pt) AND (SELECT petlimit FROM caretakers c WHERE NEW.username_caretaker = c.username) > (SELECT COUNT(*) FROM bids WHERE startdate = NEW.startdate AND enddate = NEW.enddate AND OLD.accepted = True) THEN NEW.accepted = True; ELSE raise EXCEPTION ''pet limit reached''; END IF; RETURN NEW; END; ' 
LANGUAGE plpgsql;
...
-- RIGGER AFTER INSERT INTO REVIEWS -> UPDATE TABLE CARETAKERS ON RATING
...
-- POST /bids?transfermethod=deliver&paymentmethod=123&petname=emma&username_caretake=Duc&startdate=27102020&enddate=28102020 , addBid
